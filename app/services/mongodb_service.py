...
class MongoDBService:
    ...
    def load_env_vars(self):
        load_dotenv()
        self.total_machines = int(os.getenv('TOTAL_MACHINES', 1))
    ...
    def init_mongo_connections(self):
        ...
        self.replicate_uri1 = os.getenv('MONGO_CONNECTION_STRING_REPLICA_1') or self.build_mongo_uri('MONGO_HOSTS_1', 'MONGO_OPTS_REPLICA_1')
        self.replicate_uri2 = os.getenv('MONGO_CONNECTION_STRING_REPLICA_2') or self.build_mongo_uri('MONGO_HOSTS_2', 'MONGO_OPTS_REPLICA_2')
    ...
    def compare_and_update(self, db_name=None, collection_name=None, upsert_key=None):
        if db_name is not None and collection_name is not None:
            ...
            self.sync_collection(upsert_key)
    ...
    def sync_collection(self, upsert_key=None):
        ...
        logger.info(f'[{self.machine_id}] Sync started for database {self.db_name}: {self.total_docs} estimated total documents in collection {self.collection_name}')
        logger.info(f'[{self.machine_id}] Syncing {self.db_name}.{self.collection_name} with {self.max_workers} workers')
        ...
        logger.info(f'[{self.machine_id}] Sync ended for {self.db_name}.{self.collection_name}')
        logger.info(f'[{self.machine_id}] Cleaned up {batch_file}. Closed connections to databases and exiting...')
    ...
    def replicate_changes(self, db_name=None, collection_name=None):
        ...
        collection_src = self.get_collection(self.db_name, self.collection_name, self.syncSrc_replicate)
        collection_dst = self.get_collection(self.db_name, self.collection_name, self.syncDst_replicate)
